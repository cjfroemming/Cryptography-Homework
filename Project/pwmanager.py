...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QMainWindow, QPushButton, QVBoxLayout, QLineEdit, QLabel, QScrollArea, QHBoxLayout, QVBoxLayout, QFormLayout)
...
class LoginWindow(QMainWindow) :
    ...
    def login_button_pressed(self):
        ...
        if len(self.login_attempts[username]) >= 3:  # Allow max 3 attempts in 30 seconds
            self.message_label.setText("Too many attempts. Please wait 30 seconds before trying again.")
            QTimer.singleShot(30000, lambda: self.message_label.setText("Please login"))  # Go back to original message after 30 seconds.
            return
    ...
    def create_account_button_pressed(self):
        ...
        if user_exists_in_master_database(username) : 
            ...
            if (len(password) < 8):
                self.message_label.setText('Password must be at least 8 characters long.')
            else:
...
class PasswordWindow(QMainWindow) :
    ...
    def init_buttons(self):
        ...
        self.window_layout.addWidget(self.logout_button) #added to the top of the window.
    ...
    def create_new_account(self):
        ...
        user = self.get_user()
        add_to_password_database(user,new_UID,'','','','','','','') #add new account to database.
...
class PasswordInterface(QWidget) :
    ...
    def init_buttons(self):
        ...
        self.edit_or_save_button = QPushButton('Edit') #alternates between being an edit button and a save button.
    ...
    def saveDataToDatabase(self):
        ...
        filtered_database = password_database.loc[(password_database['User'] == self.__user__) & (password_database['Password UID'] == self.__uid__)]
...
def encrypt_string(string, salt) :
    ...
    string_length = bytes(str(len(string)), 'utf-8') #needs to be in bytes for json file.
    header =b'header'
    cipher.update(header)
    ciphertext, tag = cipher.encrypt_and_digest(bytes((string + str(salt)),'utf-8'))

    json_k = ['nonce', 'header', 'ciphertext', 'tag', 'string length']
    json_v = [b64encode(x).decode('utf-8') for x in (cipher.nonce, header, ciphertext, tag, string_length) ] #nonce is 16 bytes.
...
def decrypt_string(json_input):
    ...
    plaintext = cipher.decrypt_and_verify(jv['ciphertext'], jv['tag']) #decrypt and verify the information is correct.
